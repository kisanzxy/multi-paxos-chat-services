...
class Replica:
    ...
    # heartbeat msg and view change
    # def heartbeat_send(self, targets):
    #     while self.is_live:
    #         for target in targets:
    #             send(
    #                 target.host,
    #                 target.port,
    #                 {'type': HEARTBEAT, 'proposdal_id': self.proposal_id, 'proposer_id': self.proposer_id})
    #         time.sleep(1)




    # # get request from client, propose
    # def process_request(self, request_msg):
    #     request_id = get_request_id(request_msg) # client sequence num
    #     client_id = get_client_id(request_msg)   # client id
    #     value = get_value(request_msg)

    #     if request_id in self.processed_request: # already processed
    #         #send ack
    #         pass
        
    #     if client_id<len(self.client_list):
    #         # create new thread to propose and wait
    #         self.proposer.propose(value, self.slot_num, self.replica_list) # propose this value
    #         self.slot_num += 1
        
    # # get proposal from proposer, accpect if view
    # def process_decree(self, decree_msg):
    #     sender = get_sender_id(decree_msg)
    #     if (sender != self.view):   # if not from current leader, throw
    #         return

    #     # acceptor process decree msg
    #     self.acceptor.process_proposal(decree_msg)

    # # get acceptance from acceptor, update learner
    # def process_acceptance(self, acceptance_msg):
    #     if self.learner.process_acceptance(acceptance_msg): # if can learn after this msg
    #         self.commit(acceptance_msg) # learn
    #     else:
    #         pass # do nothing
    
    # def commit(self, acceptance_msg):
    #     slot_num = get_slot_num(acceptance_msg)
    #     value = get_value(acceptance_msg)
    #     self.log[slot_num] = value
    #     self.slot_num = slot_num + 1
